...
import type { IntuitionTask } from "@/data/lessons";
...
export function IntuitionCard({ task, onSolved }: Props) {
  ...
  const check = () => {
    const ok = order.join("|") === task.correctOrder.join("|");
    setState(ok ? "correct" : "wrong");
    if (ok) onSolved();
  };

  const broken = useMemo(() => task.brokenSentence, [task.brokenSentence]);
    ...
    <div className="space-y-5">
      ...
      >
        <div className="font-hanzi flex flex-wrap items-center gap-2 text-2xl">
          <AnimatePresence>
            {order.map((origIdx, i) => (
              <motion.div key={origIdx} layout className="flex flex-col items-center gap-1">
        ...
        <p className="mt-3 font-mono text-xs text-muted-foreground">{task.translation}</p>
      </div>
